...
int main(void)
{
    ...
    struct usart_conf k_uart_1_conf;
    k_uart_1_conf.device = NULL;
    k_uart_1_conf.baudrate = 9600;
    k_uart_1_conf.databits = 8;
    k_uart_1_conf.paritysetting = HAL_UART_PARITY_NONE;
    k_uart_1_conf.stopbits = HAL_UART_STOP_BITS_1;
    k_uart_1_conf.checkparity = 0;

    usart_init(&k_uart_1_conf);

    for(int ii = 0; ii < 10; ii++)
      usart_putc('T');

    for(int ii = 0; ii < 10; ii++)
      printf("Test Receive: %c\n",usart_getc());
